<template>
  <div class="product-page">
    <div class="left">
      ...
      <img src="../assets/steam.jpg" alt="商品图片" class="product-image" />
    </div>

    <div class="right">
      <h2 class="product-title">Steam 礼品卡</h2>
      <div class="price">
        商品单价：<span class="text-green-500">¥{{ steamPrice }}</span>
      </div>
      <div class="status">
        发货方式：<span class="status-tag">在线发货</span>
      </div>
      <div @click.stop="copylinkToClipboard" class="share">
        <span>分享链接给朋友</span>
      </div>
...
import { copylinkToClipboard, getSteamPrice } from "@/services/home.ts";
...
const steamPrice = ref(0);
const userEmail = loginEmailStore();

onMounted(async () => {
  captchaCode.value = realCode();
  steamPrice.value = await getSteamPrice(userEmail.getEmail);
});

const buyCount = ref(1);
const totalMoney = computed(() => {
  return buyCount.value * steamPrice.value;
});
