...
import { copylinkToClipboard, getGirlPhotoPrice } from "@/services/home.ts";
import { loginEmailStore } from "@/stores/local.ts";
import { Toaster } from "vue-sonner";
import GirlCarousel from "@/components/GirlCarousel.vue";

const girlPhotoPrice = ref(0);
const inputCode = ref("");
...
onMounted(async () => {
  ...
  girlPhotoPrice.value = await getGirlPhotoPrice(emailStore.getEmail);
});

const buyCount = ref(1);
const totalMoney = computed(() => {
  return buyCount.value * girlPhotoPrice.value;
});
...
<template>
  <div class="product-page">
    ...
    <div class="right">
      <h2 class="product-title">萝莉写真集</h2>
      <div class="price">
        商品单价：<span class="text-green-500">¥{{ girlPhotoPrice }}</span>
      </div>
      <div class="status">
