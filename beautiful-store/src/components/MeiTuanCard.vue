...
import { copylinkToClipboard, getXieChengPrice } from "@/services/home.ts";
import { loginEmailStore } from "@/stores/local.ts";
import { Toaster } from "vue-sonner";

const xiechengPrice = ref(0);
const inputCode = ref("");
...
onMounted(async () => {
  ...
  xiechengPrice.value = await getXieChengPrice(emailStore.getEmail);
});

const buyCount = ref(1);
const totalMoney = computed(() => {
  return buyCount.value * xiechengPrice.value;
});
...
<template>
  <div class="product-page">
    <div class="left">
      ...
      <img src="../assets/xiecheng.jpg" alt="商品图片" class="product-image" />
    </div>

    <div class="right">
      <h2 class="product-title">携程酒店优惠券</h2>
      <div class="price">
        商品单价：<span class="text-green-500">¥{{ xiechengPrice }}</span>
      </div>
      <div class="status">
